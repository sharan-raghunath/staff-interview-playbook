# Day 11 — Idempotent Consumers, Durable Uniqueness and Merge Two Sorted Lists

## Focus

Day 11 continued from Day 10's Transactional Outbox learning.

Day 10 established:

> Outbox prevents lost messages.

Day 11 focused on the matching consumer-side question:
...
## 1. Day 10 Reminder: Outbox Creates At-least-once Publishing
...
During Day 11, the user pointed out that adding every stage status as a new column on `Jobs` would make the table keep growing.
...
`JobStages` was introduced on Day 11 as a schema design decision. It was not assumed or decided during Days 8–10.
...
- We reset the language and then explicitly made `JobStages` a Day 11 schema decision.
